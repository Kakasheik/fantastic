...
export function PaymentStep({ creator, plan, onClose }: Props) {
  const [copied, setCopied] = useState(false);

  const checkout = useMutation<CheckoutResult, Error, 'PIX' | 'CREDIT_CARD' | 'GOOGLE_PAY' | 'PICPAY' | 'WALLET'>({
    mutationFn: (method) =>
  ...
  // Inicia automaticamente com Pix
  if (!checkout.data && !checkout.isPending && !checkout.error) {
    checkout.mutate('PIX');
  }
  ...
  if (checkout.isPending && !checkout.data) {
    return <div className="py-10 text-center text-muted text-sm">Gerando pagamento...</div>;
  }

  if (checkout.error) {
    return (
      <div className="py-6 text-center space-y-3">
        <p className="text-red-600 text-sm">Não foi possível gerar o pagamento.</p>
        <button onClick={() => checkout.mutate('PIX')} className="text-brand text-sm underline">
          Tentar novamente
        </button>
      </div>
    );
    ...
    <div className="space-y-4">
      ...
      <div className="border-t border-border pt-4 space-y-2">
        <button
          ...
          className="w-full h-12 rounded-full bg-text text-bg font-medium flex items-center justify-center gap-2 hover:bg-text/90"
        >
        ...
        <button
          ...
          className="w-full h-12 rounded-full bg-surface2 hover:bg-border text-text font-medium flex items-center justify-center gap-1.5"
        >
